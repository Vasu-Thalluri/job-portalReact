...
export default function ApplyToJob() {
  ...
  const locations = jobs
    .filter((item) => item.id == selectedCompany)
    .map((item) => item.location);
  const jobData = jobs
    .filter(
      (item) => item.id == selectedCompany && item.location == selectedLocation
    )
  ...
  const visibleJobs =
    role === "admin"
      ? applications
      : applications.filter(
          (application) => application.user_id === user.userId
        );
  ...
  const applyToJob = async () => {
    ...
    const jobSkills = skills
      ...
      .filter((skill) => skill);
    try {
      const res = await axios.post(`${API_URL}/apply/job`, {
        userId,
        jobId: selectedCompany,
        coverLetter,
        skills: jobSkills,
      });
      //console.log(res);
  ...
  const updateAppliedJob = async () => {
    const updatedSkills = skills
      ...
      .filter((skill) => skill);
    try {
      const res = await axios.put(`${API_URL}/apply/updateJob`, {
        id: appId,
        jobId: selectedCompany,
  ...
  const updateStatus = (id, status) => {
    ...
    axios
      .put(`${API_URL}/apply/${id}/status`, { status })
      .then((res) => {})
      .catch((err) => {});
    window.location.reload();
  };
  ...
  return (
    <div>
      ...
      <form
        action=""
        onSubmit={isEditing ? handleUpdateAppliedJob : handleApplyToJob}
        style={{
          display: "flex",
          alignItems: "center",
        ...
        <div>
          ...
          >
            ...
            <input
              ...
              placeholder="Enter skill with , seperated"
              required
            />
          </label>
        </div>
        ...
        <div>
          ...
          >
            ...
            <input
              ...
              placeholder="Describe strength on skills"
            />
          </label>
        </div>
        {/* {user.role==='admin' &&
                  ...
                  {user.role === "user" && (
                    <td>
                      <button
                        ...
                        style={{ padding: "1px 5px", marginTop: "25px" }}
                      >
                        Update
                      </button>
                    </td>
